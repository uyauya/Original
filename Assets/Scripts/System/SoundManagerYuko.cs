...
public class SoundManagerYuko : SingletonMonoBehaviour<SoundManagerYuko> {
	...
	private void Start()
	{
		Yuko.GetComponent<Slider> ().value = Volume ();
		Yuko.GetComponent<Slider> ().onValueChanged.AddListener ((value) => {
			Value = value;
	...
	{
		Value = Yuko.GetComponent<Slider>().value;
		foreach(var SoundSource in _voiceAudioSource)
		{
			SoundSource.volume = Value;
		}
