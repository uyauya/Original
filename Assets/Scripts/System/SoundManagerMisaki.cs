...
//みさきの声管理用（どのSceneでも情報を保持したい場合MonoBehaviourをSingletonMonoBehaviour<スクリプト名>にする）
public class SoundManagerMisaki : SingletonMonoBehaviour<SoundManagerMisaki> {
	...
	private void Start()
	{
		Misaki.GetComponent<Slider> ().value = Volume ();
		Misaki.GetComponent<Slider> ().onValueChanged.AddListener ((value) => {
			Value = value;
	...
	{
		Value = Misaki.GetComponent<Slider>().value;
		foreach(var SoundSource in _voiceAudioSource)
		{
			SoundSource.volume = Value;
		}
